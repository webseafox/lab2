#!/bin/bash

set -euo pipefail

umask 0022

export CI=true
export npm_config_loglevel="${npm_config_loglevel:-error}"
export NPM_CONFIG_LOGLEVEL="${NPM_CONFIG_LOGLEVEL:-error}"

# Garantir que o cache do npm seja gravável quando o container não roda como root.
export HOME="${HOME:-/tmp}"
export XDG_CACHE_HOME="${XDG_CACHE_HOME:-/tmp/.cache}"
export npm_config_cache="${npm_config_cache:-/tmp/.npm}"
export NPM_CONFIG_CACHE="${NPM_CONFIG_CACHE:-${npm_config_cache}}"
# Garante cache gravável antes de instalar dependências ou iniciar o navegador.
mkdir -p "${HOME}" "${XDG_CACHE_HOME}" "${npm_config_cache}"
chmod -R u+rwX "${HOME}" "${XDG_CACHE_HOME}" "${npm_config_cache}" 2>/dev/null || true

PROJECT_DIR="$(pwd)"
RUN_RESULTS_DIR="${RUN_RESULTS_DIR:-/app/RunResults}"
LOG_DIR="${RUN_RESULTS_DIR}/logs"
REPORTS_DIR="${REPORTS_DIR:-reports}"

mkdir -p "${RUN_RESULTS_DIR}" \
         "${LOG_DIR}" \
         "${RUN_RESULTS_DIR}/reports"

restore_workspace_permissions() {
  # Garante que os arquivos gerados dentro do container possam ser removidos pelo agente host.
  local target_uid="${HOST_UID:-}"
  local target_gid="${HOST_GID:-}"

  set +e

  if [[ "${target_uid}" =~ ^[0-9]+$ && "${target_gid}" =~ ^[0-9]+$ ]]; then
    echo "🔧 [FIX] Restaurando ownership para ${target_uid}:${target_gid}..."
    chown -R "${target_uid}:${target_gid}" "${PROJECT_DIR}" 2>/dev/null || true
  else
    echo "ℹ️ [INFO] HOST_UID/HOST_GID não informados; pulando ajuste de ownership"
  fi

  chmod -R u+rwX "${PROJECT_DIR}" 2>/dev/null || true
}

trap restore_workspace_permissions EXIT

NODE_RUNNER=()
if [ "$(id -u)" -eq 0 ]; then
  if command -v runuser >/dev/null 2>&1 && id -u node >/dev/null 2>&1; then
    NODE_RUNNER=(runuser -u node --)
  else
    echo "❌ [SECURITY] Container iniciou como root, mas não foi possível reduzir privilégios para o usuário node."
    echo "   Garanta que a imagem contenha o usuário node e o binário runuser."
    exit 1
  fi
fi

log_step() {
  echo ""
  echo "=================================================="
  echo "${1}"
  echo "=================================================="
}

warn() {
  echo "##vso[task.logissue type=warning]${1}"
  echo "⚠️  [WARN] ${1}"
}

die() {
  echo "##vso[task.logissue type=error]${1}"
  echo "❌ [ERROR] ${1}"
  exit 1
}

normalize_bool() {
  local value="${1:-false}"
  echo "${value}" | tr -d '\r' | xargs | tr '[:upper:]' '[:lower:]'
}

trim_value() {
  local value="${1:-}"
  echo "${value}" | tr -d '\r' | xargs
}

require_command() {
  local command_name="${1:-}"

  if ! command -v "${command_name}" >/dev/null 2>&1; then
    die "Dependência obrigatória não encontrada: ${command_name}"
  fi
}

is_positive_int() {
  [[ "${1:-}" =~ ^[1-9][0-9]*$ ]]
}

is_non_negative_int() {
  [[ "${1:-}" =~ ^[0-9]+$ ]]
}

is_supported_environment() {
  [[ "${1:-}" == "dev" || "${1:-}" == "hom" ]]
}

ENVIRONMENT="$(trim_value "${ENVIRONMENT:-dev}")"
ENVIRONMENT="$(echo "${ENVIRONMENT}" | tr '[:upper:]' '[:lower:]')"
HEADLESS="$(normalize_bool "${HEADLESS:-true}")"
CI="true"
TAGS="$(trim_value "${TAGS:-${TEST_TAGS:-}}")"
CUCUMBER_PROFILE="$(trim_value "${CUCUMBER_PROFILE:-${TEST_SUITE:-}}")"
RETRY_COUNT="$(trim_value "${RETRY_COUNT:-0}")"
PARALLEL_WORKERS="$(trim_value "${PARALLEL_WORKERS:-2}")"
BROWSER="$(trim_value "${BROWSER:-chromium}")"

export ENVIRONMENT HEADLESS CI TAGS CUCUMBER_PROFILE RETRY_COUNT PARALLEL_WORKERS BROWSER


EXECUTION_MODE="default"
SELECTED_PROFILE="default"

# Tags sempre têm prioridade porque são o seletor mais explícito da execução.
if [ -n "${TAGS}" ]; then
  EXECUTION_MODE="tags"
elif [ -n "${CUCUMBER_PROFILE}" ]; then
  EXECUTION_MODE="profile"
  if [ "${CUCUMBER_PROFILE}" = "all" ]; then
    SELECTED_PROFILE="default"
  else
    SELECTED_PROFILE="${CUCUMBER_PROFILE}"
  fi
elif is_supported_environment "${ENVIRONMENT}"; then
  EXECUTION_MODE="profile"
  SELECTED_PROFILE="${ENVIRONMENT}"
fi

if [ ! -f package.json ]; then
  die "package.json nao encontrado no diretorio atual (${PROJECT_DIR})"
fi

if [ ! -f cucumber.js ]; then
  die "cucumber.js nao encontrado no diretorio atual (${PROJECT_DIR})"
fi

if [ ! -d features ]; then
  die "Diretorio features/ nao encontrado"
fi

if [ ! -d src/step-definitions ]; then
  die "Diretorio src/step-definitions/ nao encontrado"
fi

if [ ! -f ".${ENVIRONMENT}.yml" ]; then
  warn "Arquivo .${ENVIRONMENT}.yml nao encontrado. O projeto vai usar os defaults do ConfigLoader."
fi

require_command node
require_command npm
require_command npx

# ==================================================
# Cabeçalho da execução
# ==================================================
log_step "📦 [START] Execução de Testes Playwright + Cucumber"
echo "🔎 Contexto da execução"
echo "📁 Projeto           : ${PROJECT_DIR}"
echo "🌍 Environment       : ${ENVIRONMENT}"
echo "🧭 Execution Mode    : ${EXECUTION_MODE}"
echo "📋 Profile           : ${SELECTED_PROFILE}"
echo "🏷️ Tags              : ${TAGS:-<none>}"
echo "🖥️ Headless          : ${HEADLESS}"
echo "🌐 Browser           : ${BROWSER}"
echo "📦 Run Results       : ${RUN_RESULTS_DIR}"
echo "🗂️ Reports           : ${REPORTS_DIR}"
echo ""
# ==================================================
# Proxy e instalação das dependências
# ==================================================
echo ""
echo "🧹 [STEP] Limpando variáveis de proxy..."
unset http_proxy https_proxy HTTP_PROXY HTTPS_PROXY NO_PROXY
echo "✅ [OK] Proxies removidos"

echo "📦 [STEP] Instalando dependências..."
if [ -f package-lock.json ]; then
  echo "🔒 Usando npm ci (modo CI)"

  if ! "${NODE_RUNNER[@]}" npm ci --no-audit --no-fund --loglevel=error; then
    warn "npm ci falhou; tentando npm install"
    "${NODE_RUNNER[@]}" npm install --no-audit --no-fund --loglevel=error
  fi
else
  echo "ℹ️ package-lock.json não encontrado, usando npm install"

  "${NODE_RUNNER[@]}" npm install --no-audit --no-fund --loglevel=error
fi

echo "✅ [OK] Dependências instaladas"

# Valida a presença do Oracle Instant Client e exibe informações de diagnóstico do runtime.
echo ""
echo "🔐 [STEP] Validando Oracle Runtime..."

if [ -d "/opt/oracle/instantclient_23_8" ]; then
  echo "✅ [OK] Oracle Instant Client detectado"
else
  warn "Oracle Instant Client não localizado"
fi

echo ""

# Coleta versões das principais dependências para facilitar troubleshooting.
echo ""
echo "📊 [STEP] Coletando diagnósticos de runtime..."
NODE_VERSION="$(${NODE_RUNNER[@]} node -v)"
NPM_VERSION="$(${NODE_RUNNER[@]} npm -v)"
PLAYWRIGHT_VERSION="$(${NODE_RUNNER[@]} npx playwright --version)"
CUCUMBER_VERSION="$(${NODE_RUNNER[@]} npx cucumber-js --version 2>/dev/null | tail -n 1)"

echo "Node Version     : ${NODE_VERSION}"
echo "NPM Version      : ${NPM_VERSION}"
echo "Playwright       : ${PLAYWRIGHT_VERSION}"
echo "Cucumber Version : ${CUCUMBER_VERSION}"

echo ""


# Evita publicar relatórios Playwright inexistentes, vazios ou sem testes executados.
should_publish_playwright_report() {
  local report_dir="${1:-playwright-report}"
  local index_file="${report_dir}/index.html"

  # Publica somente quando o HTML principal existe e contém resultados válidos.
  if [ ! -d "${report_dir}" ] || [ ! -f "${index_file}" ]; then
    return 1
  fi

  if grep -q -i "No tests found" "${index_file}" 2>/dev/null; then
    return 1
  fi

  return 0
}


# ==================================================
# Browser do Playwright
# ==================================================
echo ""
echo "🧪 [STEP] Navegadores Playwright"
echo "✅ [OK] Utilizando browsers pré-instalados na imagem"

CUCUMBER_ARGS=()
# Prioridade de execução: TAGS > CUCUMBER_PROFILE > ENVIRONMENT > DEFAULT.
if [ -n "${TAGS}" ]; then
  EXECUTION_MODE="tags"
  CUCUMBER_ARGS+=("--tags" "${TAGS}")
elif [ -n "${CUCUMBER_PROFILE}" ]; then
  EXECUTION_MODE="profile"

  if [ "${CUCUMBER_PROFILE}" = "all" ]; then
    SELECTED_PROFILE="default"
  else
    SELECTED_PROFILE="${CUCUMBER_PROFILE}"
    CUCUMBER_ARGS+=("--profile" "${CUCUMBER_PROFILE}")
  fi
elif is_supported_environment "${ENVIRONMENT}"; then
  EXECUTION_MODE="profile"
  SELECTED_PROFILE="${ENVIRONMENT}"
  CUCUMBER_ARGS+=("--profile" "${ENVIRONMENT}")
fi

if is_positive_int "${PARALLEL_WORKERS}"; then
  CUCUMBER_ARGS+=("--parallel" "${PARALLEL_WORKERS}")
else
  warn "PARALLEL_WORKERS invalido (${PARALLEL_WORKERS}); mantendo a configuracao do cucumber.js"
fi

if is_non_negative_int "${RETRY_COUNT}"; then
  if [ "${RETRY_COUNT}" -gt 0 ]; then
    CUCUMBER_ARGS+=("--retry" "${RETRY_COUNT}")
  fi
else
  warn "RETRY_COUNT invalido (${RETRY_COUNT}); mantendo a configuracao do cucumber.js"
fi

# ==================================================
# Execução dos testes
# ==================================================
case "${EXECUTION_MODE}" in
  tags)
    echo "🏷️ Estratégia de execução: TAGS"
    echo "🏷️ TAGS: ${TAGS}"
    ;;
  profile)
    echo "📋 Estratégia de execução: PROFILE"
    echo "📋 Profile selecionado: ${SELECTED_PROFILE}"
    if [ "${SELECTED_PROFILE}" = "default" ]; then
      echo "ℹ️ Executando todos os cenários"
    fi
    ;;
  *)
    echo "📋 Estratégia de execução: DEFAULT"
    ;;
esac

echo ""
echo "▶️ [STEP] Executando testes..."
FINAL_CUCUMBER_COMMAND="cucumber-js"
if [ "${#CUCUMBER_ARGS[@]}" -gt 0 ]; then
  for arg in "${CUCUMBER_ARGS[@]}"; do
    FINAL_CUCUMBER_COMMAND="${FINAL_CUCUMBER_COMMAND} $(printf '%q' "${arg}")"
  done
fi
echo "▶️ Comando final: ${FINAL_CUCUMBER_COMMAND}"

set +e

"${NODE_RUNNER[@]}" npx cucumber-js "${CUCUMBER_ARGS[@]}" 2>&1 | tee "${LOG_DIR}/cucumber.log"

CUCUMBER_EXIT_CODE=${PIPESTATUS[0]}

set -e

if command -v node >/dev/null 2>&1 && [ -f "scripts/generate-report.js" ]; then
  # Gera o índice antes da consolidação para que o artefato publicado reflita a versão final.
  echo ""
  echo "📊 [STEP] Gerando indice de relatorios..."
  "${NODE_RUNNER[@]}" node scripts/generate-report.js || warn "Falha ao gerar o indice de relatorios"
fi

# ==================================================
# Consolidação de evidências
# ==================================================
echo ""
echo "🧾 [STEP] Consolidando artefatos..."

if [ -d "${REPORTS_DIR}" ]; then
  # Aceita diretórios de relatório definidos pelo próprio projeto.
  cp -a "${REPORTS_DIR}"/. "${RUN_RESULTS_DIR}/reports"/ 2>/dev/null || true
fi
if should_publish_playwright_report "playwright-report"; then
  # Esse bloco fica separado para não duplicar nem sobrescrever o relatório padrão quando REPORTS_DIR já aponta para ele.
  mkdir -p "${RUN_RESULTS_DIR}/reports/playwright-report"
  cp -a "playwright-report"/. "${RUN_RESULTS_DIR}/reports/playwright-report"/ 2>/dev/null || true
elif [ -d "playwright-report" ]; then
  echo "ℹ️ [INFO] Playwright report vazio; publicação ignorada"
fi

if [ -f "${REPORTS_DIR}/cucumber-report.html" ] || [ -f "${REPORTS_DIR}/index.html" ] || [ -f "playwright-report/index.html" ] || [ -f "reports/index.html" ]; then
  echo "✅ [OK] Relatorios encontrados"
else
  warn "Nenhum relatorio HTML padrao encontrado"
fi

# Mantém o mesmo padrão do Cypress para evitar problema de permissão no host.
chmod -R a+rX,u+w "${RUN_RESULTS_DIR}" 2>/dev/null || true

# ==================================================
# Metadata e finalização
# ==================================================
echo ""
echo "✅ Cucumber finalizado com código: ${CUCUMBER_EXIT_CODE}"
echo "📦 Evidências em: ${RUN_RESULTS_DIR}"

# ==================================================
# Mensagem final
# ==================================================
if [ "${CUCUMBER_EXIT_CODE}" -ne 0 ]; then
  echo "❌=================================================="
  echo "❌ [FAILED] Execução de testes Playwright com falhas"
  echo "❌ Evidências disponíveis em ${RUN_RESULTS_DIR}"
  echo "❌=================================================="
  echo ""
else
  echo "🎉=================================================="
  echo "🎉 [SUCCESS] Execução de testes Playwright concluída"
  echo "🎉 Evidências disponíveis em ${RUN_RESULTS_DIR}"
  echo "🎉=================================================="
  echo ""
fi

exit ${CUCUMBER_EXIT_CODE}