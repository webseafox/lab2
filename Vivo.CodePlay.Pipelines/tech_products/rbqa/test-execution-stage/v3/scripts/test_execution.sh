...
function read_scenarios() {
  ...
  if [[ ${scenarios} == "default"* ]]; then
    [[ ! -s /app/${COMPONENT_ID}/.azuredevops/cenarios/$scenarios.txt ]] && echo -e "[$(date '+%Y-%m-%d %H:%M:%S')][ERROR] Tests $scenarios.txt file is not found." >&2 && exit 1
    scenarios=$(cat /app/${COMPONENT_ID}/.azuredevops/cenarios/"$scenarios".txt)
  fi
...
function read_optionals() {
  ...
  if [[ ${optionals} == "default"* ]]; then
    [[ ! -s /app/${COMPONENT_ID}/.azuredevops/params/$optionals.txt ]] && echo -e "[$(date '+%Y-%m-%d %H:%M:%S')][ERROR] Params $optionals.txt file is not found." >&2 && exit 1
    optionals=$(cat /app/${COMPONENT_ID}/.azuredevops/params/"$optionals".txt)
  fi
...
function maven_command() {
  ...
  local LOG_FILE_NAME="${LOG_FILE_NAME}_${ENVIRONMENT}"
  mkdir /tmp/"${CENARIO}"
...
if [[ ${#SCENARIOS_BLOCKS[@]} -eq 0 ]]
then
  echo "[$(date '+%Y-%m-%d %H:%M:%S')][ERROR] Nenhum cenário foi carregado. Verifique a configuração."
  exit 1
fi
mapfile -t OPTIONALS_BLOCKS <<<$(read_optionals "${OPTIONALS}")
mapfile -t PROJECT_IDS <<<$(read_projectid "${PROJECT_ID}")
echo "Pasta /app/COMPONENT_ID/.azuredevops/params"
ls -lha /app/${COMPONENT_ID}/.azuredevops/params
