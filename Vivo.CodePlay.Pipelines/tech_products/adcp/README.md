...
## Introdução
Este repositório contém definições de pipelines do Azure DevOps para compilar, testar, proteger e implantar aplicações Flutter para Vivo+. As pipelines suportam as plataformas Android e iOS em múltiplos ambientes (esteira1, preprod, release, prodlike e producao) e incluem scanners de segurança, proteção do app e publicação em lojas.
...
- **Funcionalidades**:
  - Carregamento de variáveis específicas do ambiente (esteira1, preprod, release, prodlike e producao)
...
#### **Android Packaging**
- **APK Generation**: 
  - Configuração de ambiente específico (esteira1, preprod, release, prodlike e producao)
...
- **Configuração dos arquivos para publicação Firebase (Android)**:
  - Configuração dos arquivos para publicação firebase (esteira1, preprod, prodlike e producao):
...
| Branch / Tag de origem | Tag gerada |
|------------------------|------------|
| `master` | `PRE-<build>` |
| Branch definida em `prod_source_branch` | `PRE-<build>` |
| `release` | `RELEASE-PRE-<build>` |
| Tag `RELEASE-PRE-*` | `RELEASE-<build>` |
| Tag `PRE-*` | `PRD-<build>` |

#### Parâmetro `prod_source_branch`

Define uma branch adicional autorizada a gerar a tag `PRE-`, que é a tag exigida pelo stage
de deploy para os ambientes `prodlike` e `producao`.

- **Default**: `master` — mantendo o comportamento anterior à existência do parâmetro.
- **Formato**: nome da branch sem o prefixo `refs/heads/` (ex.: `hotfix/pti-8240`).
- **Efeito**: a branch informada passa a ser aceita pelo gate `Check Branch/Tag & Environment`
  em conjunto com o ambiente `preprod`, e a gerar a tag `PRE-` no stage de Tag.
- **Precedência**: `master`, `release`, `RELEASE-PRE-*` e `PRE-*` são avaliados antes. Apontar
  `prod_source_branch` para `release` não altera o comportamento dessa branch, que continua
  gerando `RELEASE-PRE-`.
- **Escopo**: o parâmetro não altera as condições do stage de Deploy, que seguem exigindo
  tag `PRE-*` ou `RELEASE-PRE-*` e a aprovação do `environment` correspondente.
...
### 🛡️ **Arxan (Android)**
- **Quando aplicada**: exclusivamente quando `skip_protect = False`, em qualquer ambiente
- **Atenção**: até a introdução do controle exclusivo por `skip_protect`, a proteção era
  aplicada automaticamente nos ambientes `release`, `prodlike` e `producao` mesmo com
  `skip_protect = True`. Esse forçamento por ambiente não existe mais — o pacote protegido
  passa a ser selecionado apenas pelo valor de `skip_protect`.
- **Funcionalidades**:
...
A pipeline suporte 4 ambientes distintos:

| Ambiente | Descrição | Proteção | Distribuição |
|----------|-----------|----------|--------------|
| **esteira1** | Ambiente de desenvolvimento | Opcional | Firebase apenas |
| **preprod** | Pré-produção/Homologação | Opcional | Firebase + TestFlight |
| **release** | Release candidate | Conforme `skip_protect` | Firebase + Lojas |
| **prodlike** | Produção-like | Conforme `skip_protect` | Firebase |
| **producao** | Produção | Conforme `skip_protect` | Firebase + Lojas |
...
resources:
  repositories:
    - repository: CodePlayPipelines
      ...
      type: git
      ref: refs/heads/feature/adcp-flutter-app
      #ref: master
      endpoint: CodePlay

extends:
  template: /tech_products/adcp/pipeline_flutter_app.yml@CodePlayPipelines
  parameters:
...
### 🧪 **Esteira1/Preprod/Prodlike (Testes)**
```mermaid
graph LR
...
1. **Documentação Técnica**: Consulte os templates individuais em `/tech_products/adcp/templates/`
2. **Logs**: Analise os logs detalhados de cada stage no Azure DevOps
3. **Suporte**: Entre em contato com a equipe DevOps Vivo+
...
resources:
  repositories:
    - repository: CodePlayPipelines
      ...
      type: git
      # ref: master
      ref: refs/heads/feature/adcp-flutter-lib
      endpoint: CodePlay

extends:
  template: /tech_products/adcp/pipeline_flutter_lib.yml@CodePlayPipelines
  parameters:
...
resources:
  repositories:
    - repository: CodePlayPipelines
      ...
      type: git
      #ref: master
      ref: refs/heads/feature/adcp-flutter-web
      endpoint: CodePlay

extends:
  template: /tech_products/adcp/pipeline_flutter_web.yml@CodePlayPipelines
  parameters:
    bump_version: "${{parameters.bump_version}}"
    bump_message: "${{parameters.bump_message}}"  
    use_flutter_version_latest: "${{parameters.use_flutter_version_latest}}"
