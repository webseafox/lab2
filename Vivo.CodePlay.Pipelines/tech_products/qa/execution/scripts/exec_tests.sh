...
if [[ ${DOCKER_IMAGE} == "selenium" ]] || [[ ${DOCKER_IMAGE} == "selenium-v2" ]] || [[ ${DOCKER_IMAGE} == "selenium-xfcb-chrome" ]] || [[ ${DOCKER_IMAGE} == "selenium-xfcb-edge" ]] || [[ ${DOCKER_IMAGE} == "selenium-xfcb-firefox" ]]; then
  unset DISPLAY
...
# Nivel de log do Maven.
# O -X (debug) faz o Maven despejar TODAS as variaveis de ambiente no log, no
# formato "[DEBUG] env.NOME: valor" - incluindo credenciais repassadas ao
# container. Por isso ele agora so e usado quando explicitamente solicitado via
# o parametro maven_debug do template (ou com system.debug ligado).
# O -e mantem o stack trace completo em caso de erro, sem o despejo de ambiente.
if [[ "${MAVEN_DEBUG,,}" == "true" ]]; then
  MAVEN_LOG_FLAG="-X"
  echo "[$(date '+%Y-%m-%d %H:%M:%S')][WARNING] Maven em modo debug (-X). O log vai conter o despejo das variaveis de ambiente."
else
  MAVEN_LOG_FLAG="-e"
fi
...
function read_scenarios() {
  ...
  if [[ ${scenarios} == "default"* ]]; then
    [[ ! -s .azuredevops/cenarios/$scenarios.txt ]] && echo -e "[$(date '+%Y-%m-%d %H:%M:%S')][ERROR] Tests $scenarios.txt file is not found." >&2 && exit 1
    scenarios=$(cat .azuredevops/cenarios/"$scenarios".txt)
  fi
...
function read_optionals() {
  ...
  if [[ ${optionals} == "default"* ]]; then
    [[ ! -s .azuredevops/params/$optionals.txt ]] && echo -e "[$(date '+%Y-%m-%d %H:%M:%S')][ERROR] Params $optionals.txt file is not found." >&2 && exit 1
    optionals=$(cat .azuredevops/params/"$optionals".txt)
  fi
...
function maven_command() {
  ...
  cp -r /app "$AGENT_TEMPDIRECTORY"/"${CENARIO}"
  ln -s "$AGENT_TEMPDIRECTORY"/"${CENARIO}" ${AGENT_TOOLSDIRECTORY}/downloads

  export RUN_CMD="mvn $ACTION install -U ${MAVEN_LOG_FLAG} \
        -f $AGENT_TEMPDIRECTORY/"${CENARIO}"/pom.xml \
        -s $AGENT_TEMPDIRECTORY/"${CENARIO}"/settings.xml \
        "${AMBIENTE}" \
        -Dalm_user="${ALM_USER}" \
        ...
        -Drun_results_folder=$AGENT_TEMPDIRECTORY/"${CENARIO}"/RunResults  \
        -Dignore.test.failure=false \
        -Dmaven.repo.local=/.m2/repository \
        -Dorg.slf4j.simpleLogger.showDateTime=true \
        -Dorg.slf4j.simplelogger.dateTimeFormat=\"yyyy-MM-dd-HH:mm:ss\" \
        -Dstyle.color=never"
  echo "[$(date '+%Y-%m-%d %H:%M:%S')][INFO] MAVEN COMMAND: ${RUN_CMD}" | sed "s/-Dalm_password=${ALM_PASSWORD}/-Dalm_password=******/" | sed "s/-Dalm-password=${ALM_PASSWORD}/-Dalm-password=******/" | tee -a /logs/"${LOG_FILE_NAME}"_"UNFINISHED".log
  ...
  EXIT_CODE=$?
  mv "$AGENT_TEMPDIRECTORY"/"${CENARIO}"/RunResults /app/RunResults/"${LOG_FILE_NAME}"_RunResults
