...
function Invoke-PackageCloseInfo {
    try {
        ...
        Write-Host "       VIVO DEVOPS - LIMPA REPO" -ForegroundColor Yellow
        Write-Host "=" * 60 -ForegroundColor Cyan
        ...
        $successMessage = "Conseguimos limpar o repositorio $datetime"
        ...
        Write-Host "ERRO: Falha ao executar limpa repo" -ForegroundColor Red
        Write-Host "Detalhes: $($_.Exception.Message)" -ForegroundColor Red
        
        return @{
            Success = $false
            Message = "Erro ao limpar repo"
