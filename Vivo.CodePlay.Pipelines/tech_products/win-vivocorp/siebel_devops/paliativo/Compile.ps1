...
$logFile = Join-Path -Path $logPath -ChildPath "Compile_DEV1_$timestamp.log"
...
if (Test-Path -Path $tempFile) {
    ...
    $process = Start-Process -FilePath "C:\Siebel\8.1\Tools_1\BIN\siebdev.exe" `
        -ArgumentList @(
            '/c "C:\Siebel\8.1\Tools_1\BIN\ENU\DEV1.cfg"',
            '/s DEV1',
            '/u $(siebeluser)',
            '/p $(siebelpass)',
            '/tl PTB',
            '/bc "Siebel Repository"',
            'C:\Siebel_Devops\DEV1\srf\siebel_sia_new.srf'
        ) `
        -PassThru `
        -NoNewWindow
