### Instrução Geral ###
        ...
        ### 5. Melhores Práticas
        O Codigo deve estar aderente ao seguinte padrao de desenvolvimento:
            - Deve sempre haver um controller que irá acessar diretamente um metodo que invoca uma API, o consumo dessa API deverá ficar abstraido em uma dependencia maven que será importada no POM 
            - O metodo no controller deve semepre seguir a estrutura proposta no seguinte diagrama mermaid:
                sequenceDiagram
                    participant Client as Client
                    participant ServiceTeste as ServiceTeste
                    participant AppState as vars
                    participant LogApi as LogApi
                    participant TransactionConfig as TransactionConfig
                    participant ExternalService as ExternalService

                    Client ->> ServiceTeste: POST /{serviceName}(appStateIn)
                    ServiceTeste ->> AppState: context.getBean(AppState.class, apiName)
                    ServiceTeste ->> LogApi: context.getBean(LogApi.class)
                    ServiceTeste ->> AppState: appState.get("input1")
                    ServiceTeste ->> AppState: appState.get("input2")
                    ServiceTeste ->> AppState: appState.get("input3")
                    ServiceTeste ->> LogApi: logApi.log("Entradas", new StringBuilder().append("input1: ").append(input1).append("\n") ...)
                    ServiceTeste ->> ExternalService: apiCall()
                    ExternalService -->> ServiceTeste: responseMap
                    ServiceTeste ->> AppState: appState.put(key, value) for each key, value in responseVerifyMap
                    ServiceTeste ->> LogApi: logApi.setResultApi((appState.getByApiName("statCom"),appState.getByApiName("codRet"))
                    ServiceTeste ->> LogApi: logApi.logMessage(Constants.REQUEST, requestCompleto)
                    ServiceTeste ->> LogApi: logApi.logMessage(Constants.RESPONSE, responseMap.toString())
                    ServiceTeste -->> Client: ResponseEntity<>(appState.toMap(), HttpStatus.OK)    
            - Em alguns casos será aberta a excessão de criação de uma classe business para tratamento dos dados recebidos antes de inserir novamente no appState.
            - Garanta sempre que existem validações na inserção de campos no appState.put, garantindo que não ocorra um nullpointerException.
            - Garante que sempre haverá os campos de log logApi.log("Entradas", ...), logApi.setResultApi((appState.getByApiName("statCom"),appState.getByApiName("codRet")), logApi.logMessage(Constants.REQUEST, requestCompleto) e logApi.logMessage(Constants.RESPONSE, responseMap.toString())
        . **Sugira melhorias para aderir às melhores práticas** de design e código limpo.
