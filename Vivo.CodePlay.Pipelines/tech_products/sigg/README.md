...
Pipeline completo de CI para aplicações Java que gera imagens Docker, com análises de qualidade, integração com AppSec e versionamento automatizado.
...
- 🔒 Análise SAST (Fortify) com controle de execução via AppSec
...
- 🧪 Estágio opcional de testes de componente com Docker Compose
...
| [SAST](https://dvps.redecorp.azr/portal/codeplay/capacidades/catalog/sast) | ✅ | Análise estática de código com Fortify ScanCentral e SSC, condicionada pelas flags recuperadas do AppSec. Exclusões customizadas via `enableFortifyExclusions=true` e integração com Conviso via `convisoCompany`. |
| [SCA](https://dvps.redecorp.azr/portal/codeplay/capacidades/catalog/sca) | ⚠️ | O template expõe variáveis de controle vindas do AppSec para segurança/SCA no estágio Docker, mas a execução efetiva não está materializada diretamente neste YAML. Valide o comportamento no template central consumido pelo framework. |
...
O pipeline é organizado em cinco estágios, sendo um deles opcional. `JavaBuild` sempre executa primeiro. A partir dele, `SecurityAnalysis` segue no fluxo principal e `ComponentTest` pode ser habilitado em paralelo quando `runComponentTest=true`. Depois, `DockerBuild` depende de `SecurityAnalysis`, e `VersionCommit` só existe quando `prValidationOnly=false`.

Esta arquitetura prioriza build reprodutível, segurança controlada pelo AppSec e publicação condicionada por contexto de execução.

```mermaid
graph TD
  A[JavaBuild] --> B[SecurityAnalysis]
  A --> C[ComponentTest opcional]
  B --> D[DockerBuild]
  D --> E[VersionCommit opcional]
...
1. **☕ JavaBuild - Build & Test Java Application**
  ...
  - Publicação de artefatos JAR ou WAR para próximos stages via `deployableArtifactPattern`
...
2. **🧪 ComponentTest - Component Tests** (Condicional - apenas quando `runComponentTest=true`)
  ...
  - Geração de `app.tar` e publicação do artefato `app` para o template de testes
  - Execução via template `/framework/templates/component_test.yml`
...
4. **🝳 DockerBuild - Docker Build** (Depende de `SecurityAnalysis`)
   - Download de artefatos JAR do stage JavaBuild
   - Cópia de JARs para contexto Docker
  - Resolução de versão efetiva para projetos com CI-Friendly Versions (`<revision>`)
   - Build da imagem Docker com BuildKit (template `docker-build-and-push`)
  - Passa `deployableArtifactName` como `JAR_NAME` para o Dockerfile
  - Recebe variáveis de controle de segurança para decisões no estágio (`USE_DT_DOCKER`, `SKIP_SECURITY_GATE_SCA`)
...
- **`prValidationOnly=true`**: ❌ Não publica e não cria stage `VersionCommit`
...
- **tipo**: boolean
- **default**: false
- **descrição**: "Habilita suporte a projetos Maven multi-módulo. Quando `true`, os globs de reports (surefire, JaCoCo, Sonar) são automaticamente expandidos para buscar em todos os sub-módulos (`**/target/...`)."
- **dependências**: Projeto Maven com estrutura multi-módulo (`<modules>` no pom.xml pai)

#### deployableArtifactPattern

- **nome**: deployableArtifactPattern
- **tipo**: string
- **default**: `*.jar`
- **descrição**: "Padrão glob para filtrar o artefato deployável na publicação entre stages. Exemplos: `*.jar` para JARs, `*.war` para WARs, ou `*.jar,*.war` para ambos."
- **dependências**: Artefato gerado pelo Maven no diretório `target/`

#### deployableArtifactName

- **nome**: deployableArtifactName
- **tipo**: string
- **default**: `$(Build.Repository.Name)-$(VersionManagerVivo.VERSION).jar`
- **descrição**: "Nome completo do artefato deployável passado como `JAR_NAME` build arg para o Dockerfile. Para projetos multi-módulo ou WAR, ajuste para o artifactId correto (ex: `my-app-$(VersionManagerVivo.VERSION).war`)."
- **dependências**: Nome deve coincidir com o artefato gerado pelo `maven-jar-plugin` ou `maven-war-plugin`
...
#### componentTestsDirectory

- **nome**: componentTestsDirectory
- **tipo**: string
- **default**: `$(Build.SourcesDirectory)/component-tests`
- **descrição**: "Diretório base para os testes de componente. Usado como contexto para organização de arquivos e scripts de teste."
- **dependências**: Diretório deve existir no repositório quando `runComponentTest=true`
...
- **valores possíveis**:
  ...
  - ``monorepo``: Estratégia para repositórios multi-produto/monorepo
- **dependências**: Nenhuma
...
| `DevOpsSharedResources` | Azure Resource Manager | Acesso ao Azure Key Vault compartilhado, App Configuration e recursos corporativos usados pelo pipeline | Configurar no projeto: Project Settings > Service Connections > New > Azure Resource Manager |
...
- **Arquivo `.tool-versions`**: Necessário quando a versão do Java for resolvida via ASDF no diretório informado em `workingDirectory`
...
- Análise de segurança SAST (Fortify) com controles de AppSec
...
**Nota**: O pipeline consulta o AppSec para decidir execução e comportamento das rotinas de segurança; este template não materializa um gate manual local.
...
### Projeto Multi-Módulo Maven (WAR)

Para projetos Maven multi-módulo que geram um artefato WAR deployável:
...
extends:
  template: .azuredevops/template_codplay/pipeline.yaml
  parameters:
    isMultiModule: true                       # Expande globs para sub-módulos
    deployableArtifactPattern: '*.war'        # Publica WAR entre stages
    deployableArtifactName: 'sig-webservices-$(VersionManagerVivo.VERSION).war'
    workingDirectory: '$(Build.SourcesDirectory)/ms-sig-webservices-v2'
    mavenCustomSettings: '.azuredevops/settings.xml'
```

**Comportamento esperado:**
- Relatórios JUnit coletados de todos os sub-módulos (`**/target/surefire-reports/`)
- Cobertura JaCoCo agregada de todos os módulos
- SonarQube analisa binários e cobertura de todos os módulos
- Artefato WAR publicado e copiado para o stage DockerBuild
- Docker recebe `JAR_NAME=sig-webservices-X.Y.Z.war` como build arg
- `deployableArtifactName` deve casar exatamente com o nome final do artefato gerado

> **Nota**: Se este template for consumido a partir do repositório compartilhado do framework, o repositório `CodePlay` deve estar declarado em `resources` para resolver os templates centrais referenciados.

### Projeto Multi-Módulo Maven (JAR)

Para projetos multi-módulo que geram um JAR fat executável:
...
extends:
  ...
  parameters:
    isMultiModule: true
    deployableArtifactPattern: '*.jar'
    deployableArtifactName: 'my-service-$(VersionManagerVivo.VERSION).jar'
```
...
| `TEST_RESULTS_GLOB` | Glob para localizar relatórios JUnit (single ou multi-módulo) | `$workingDirectory/target/surefire-reports/*.xml` ou `$workingDirectory/**/target/surefire-reports/*.xml` |
| `JACOCO_SUMMARY_GLOB` | Glob para localizar relatórios JaCoCo XML (single ou multi-módulo) | `$workingDirectory/target/site/jacoco/jacoco.xml` ou `$workingDirectory/**/target/site/jacoco/jacoco.xml` |
| `JACOCO_REPORT_DIR` | Diretório do relatório HTML JaCoCo | `$workingDirectory/target/site/jacoco` ou `$workingDirectory` |
| `SONAR_JACOCO_GLOB` | Glob para cobertura JaCoCo no SonarQube | `$workingDirectory/target/site/jacoco/jacoco.xml` ou `$workingDirectory/**/target/site/jacoco/jacoco.xml` |
| `SONAR_BINARIES_GLOB` | Glob para binários compilados no SonarQube | `$workingDirectory/target/classes` ou `$workingDirectory/**/target/classes` |
...
### Decisão 9: Suporte a Projetos Multi-Módulo Maven
- **Data**: 02/06/2026
- **Motivador**: O template original assumia projetos single-module (globs hardcoded como `target/surefire-reports/`, `target/site/jacoco/`, `target/classes`), causando perda de relatórios de teste, cobertura e análise SonarQube em projetos Maven multi-módulo. Além disso, o filtro `*.jar` na publicação de artefatos ignorava artefatos WAR.
- **Fórum Envolvido**: Equipe SIG
- **Descrição**: Adicionados 3 novos parâmetros (`isMultiModule`, `deployableArtifactPattern`, `deployableArtifactName`) e 5 variáveis condicionais (`TEST_RESULTS_GLOB`, `JACOCO_SUMMARY_GLOB`, `JACOCO_REPORT_DIR`, `SONAR_JACOCO_GLOB`, `SONAR_BINARIES_GLOB`) que expandem os globs para `**/target/...` quando `isMultiModule=true`. A publicação de artefatos agora aceita padrões configuráveis (JAR, WAR ou ambos) e o `JAR_NAME` do Docker Build é parametrizável.
- **Impacto**: Retrocompatível — projetos single-module não precisam de nenhuma alteração (defaults mantêm o comportamento original). Projetos multi-módulo passam a ter reports, cobertura e análise SonarQube completos. A task CopyFiles no stage DockerBuild agora copia tanto `*.jar` quanto `*.war`.
- **Próximos Passos**: Avaliar proposta ao time DevOps para incorporar suporte multi-módulo no template oficial do CodePlay.
- **Referências**: Parâmetros `isMultiModule`, `deployableArtifactPattern`, `deployableArtifactName`
- **Notas**: Os defaults garantem retrocompatibilidade total: `isMultiModule=false`, `deployableArtifactPattern=*.jar`, `deployableArtifactName=$(Build.Repository.Name)-$(VersionManagerVivo.VERSION).jar`.
